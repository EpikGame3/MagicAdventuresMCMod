...
public class WaterStaffItem extends Item {
	public WaterStaffItem() {
		super(new Item.Properties().tab(MaModTabs.TAB_MAGIC_WEAPONS).durability(3126));
	}
	...
	public void releaseUsing(ItemStack itemstack, Level world, LivingEntity entityLiving, int timeLeft) {
		if (!world.isClientSide() && entityLiving instanceof ServerPlayer entity) {
			...
			if (WaterStaffCanUseRangedItemProcedure.execute(entity)) {
				WaterStaffEntity entityarrow = WaterStaffEntity.shoot(world, entity, world.getRandom(), 1f, 3, 7);

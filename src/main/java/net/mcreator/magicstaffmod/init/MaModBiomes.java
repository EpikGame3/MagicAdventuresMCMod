...
import net.mcreator.magicstaffmod.world.biome.RuneForestBiome;
import net.mcreator.magicstaffmod.world.biome.MagicForestBiome;
import net.mcreator.magicstaffmod.world.biome.FireForestBiome;
import net.mcreator.magicstaffmod.world.biome.CorruptedWastelandsBiome;
import net.mcreator.magicstaffmod.world.biome.CorruptedForestBiome;
...
public class MaModBiomes {
	...
	public static final RegistryObject<Biome> RUNE_FOREST = REGISTRY.register("rune_forest", RuneForestBiome::createBiome);
	public static final RegistryObject<Biome> FIRE_FOREST = REGISTRY.register("fire_forest", FireForestBiome::createBiome);
	public static final RegistryObject<Biome> CORRUPTED_FOREST = REGISTRY.register("corrupted_forest", CorruptedForestBiome::createBiome);
	public static final RegistryObject<Biome> CORRUPTED_WASTELANDS = REGISTRY.register("corrupted_wastelands", CorruptedWastelandsBiome::createBiome);
		...
		for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
			...
			if (dimensionType == dimensionTypeRegistry.getOrThrow(BuiltinDimensionTypes.OVERWORLD)) {
				...
				if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
					...
					parameters.add(new Pair<>(MagicForestBiome.PARAMETER_POINT,
							biomeRegistry.getOrCreateHolderOrThrow(ResourceKey.create(Registry.BIOME_REGISTRY, MAGIC_FOREST.getId()))));
				...
				if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
					...
					if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
						...
						surfaceRules.add(1, preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, MAGIC_FOREST.getId()),
			...
			if (dimensionType == dimensionTypeRegistry.getOrThrow(BuiltinDimensionTypes.NETHER)) {
				...
				if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
					List<Pair<Climate.ParameterPoint, Holder<Biome>>> parameters = new ArrayList<>(noiseSource.parameters.values());
					parameters.add(new Pair<>(FireForestBiome.PARAMETER_POINT,
							biomeRegistry.getOrCreateHolderOrThrow(ResourceKey.create(Registry.BIOME_REGISTRY, FIRE_FOREST.getId()))));
					chunkGenerator.biomeSource = new MultiNoiseBiomeSource(new Climate.ParameterList<>(parameters), noiseSource.preset);
					chunkGenerator.featuresPerStep = Suppliers
							.memoize(() -> FeatureSorter.buildFeaturesPerStep(List.copyOf(chunkGenerator.biomeSource.possibleBiomes()),
									biome -> chunkGenerator.generationSettingsGetter.apply(biome).features(), true));
				...
				if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
					...
					if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
						List<SurfaceRules.RuleSource> surfaceRules = new ArrayList<>(sequenceRuleSource.sequence());
						surfaceRules.add(2, anySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, FIRE_FOREST.getId()),
								Blocks.GRASS_BLOCK.defaultBlockState(), Blocks.DIRT.defaultBlockState(), Blocks.MAGMA_BLOCK.defaultBlockState()));
						NoiseGeneratorSettings moddedNoiseGeneratorSettings = new NoiseGeneratorSettings(noiseGeneratorSettings.noiseSettings(),
								noiseGeneratorSettings.defaultBlock(), noiseGeneratorSettings.defaultFluid(), noiseGeneratorSettings.noiseRouter(),
								SurfaceRules.sequence(surfaceRules.toArray(SurfaceRules.RuleSource[]::new)), noiseGeneratorSettings.spawnTarget(),
								noiseGeneratorSettings.seaLevel(), noiseGeneratorSettings.disableMobGeneration(),
